...
few_shots = {
    'List all customers.': 'SELECT * FROM "customer";',
    'How many orders are there?': 'SELECT COUNT(*) FROM \"order\";',
    'Help me write a query to find the number of orders placed by each customer.':
        'SELECT customer_id, COUNT(*) AS order_count FROM \"order\" GROUP BY customer_id;',
    'Help me write a query to calculate the RFM value of my customers based on their orders':
        "'Here is the SQL code to calculate the RFM (Recency, Frequency, Monetary value) of customers "
        ...
        "\nThis query provides the frequency of orders, the date of the last order, and the total monetary value "
        "of orders for each customer. The results are limited to 10 customers for brevity.'",
    "How many vendors are there?":
        "53 vendors were found. Here is the SQL query to get the count of vendors: "
        "```sql SELECT COUNT(DISTINCT \"vendor\") FROM \"product\";",
    'Help me write a query to calculate the number of orders placed by month':
        "Here is the SQL code to calculate the number of orders placed by month:"
